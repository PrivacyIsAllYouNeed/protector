...
def transcribe_simple(audio_file: str, model_size: str = "small.en"):
    ...
    try:
        with wave.open(audio_file, 'rb') as wav:
            ...
            logging.info(f"Audio info: {channels} channel(s), {sample_rate}Hz, {duration:.2f} seconds")
    ...
    try:
        ...
        # Print language info
        logging.info(f"Detected language: {info.language} (probability: {info.language_probability:.2%})")
        logging.info("-" * 50)
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python test_whisper_simple.py <audio_file.wav> [model_size] [--no-vad]")
        print("Examples:")
